...
if [[ ! -x "$binary" || ! -x "$helper" || ! -x "$release_root/redunar-update-helper" || ! -f "$release_root/libredunar_capture_vulkan.so" || ! -f "$release_root/libredunar_capture_opengl.so" || ! -x "$release_root/redunar-steam-launch" ]]; then
