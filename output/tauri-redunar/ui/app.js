...
const playbackRequests = latestReplayRequest(fileName=>call('clip_playback_path',{fileName}));
...
function overview() {
 ...
 <section class="recent-captures"><div class="section-heading"><div><h2>Recent captures</h2><p id="overview-replay-status">Waiting for replay status</p></div><a href="#replay">View all ${icon('arrow')}</a></div><div class="recent-capture-grid">${clips.slice(0,3).map(c=>`<a href="#replay" data-recent-clip="${escape(c.id)}"><span class="recent-thumbnail">${c.thumbnail?`<img src="${c.thumbnail}" alt="">`:icon('play')}<span data-clip-duration>${time(c.duration)}</span></span><span><strong title="${escape(c.file_name)}">${escape(c.file_name)}</strong><small>${escape(c.game_name||'Game not recorded')}</small><small>${escape(c.date)} · ${escape(c.size)}</small></span></a>`).join('')||'<p class="empty">Saved replays will appear here.</p>'}</div></section>`;
}
...
function discoveryModal(candidates) {
 ...
 const rows=candidates.map((candidate,index)=>{const existing=alreadyImported(candidate),importable=candidate.importable&&!existing;return `<label class="discovery-row ${importable?'':'unavailable'}"><input type="checkbox" name="index" value="${index}" ${importable?'checked':''} ${importable?'':'disabled'}><span><strong>${escape(candidate.name)}</strong><small>${escape(candidate.source)}${candidate.source_id?` · ${candidate.source_id}`:''} · ${escape(candidate.install_directory)}</small>${existing?'<em>Already in Redunar library</em>':candidate.importable?`<code>${escape(candidate.launch_executable||'')}</code>`:'<em>No verified launch found on this system</em>'}</span></label>`}).join('');
...
document.addEventListener('submit',event=>{
 ...
 if(event.target.id==='discovery-form'){event.preventDefault();const form=new FormData(event.target),indices=[...form.getAll('index')].map(Number);if(!indices.length){notify('Select at least one game to import.');return;}perform(async()=>{acceptGames(await call('import_discovered_games',{indices}));$('#dialog').close();selectedGame=games.at(-1)?.id||selectedGame;render();const count=indices.length;notify(`${count} game${count===1?'':'s'} imported into the local library.`);});return;}
 if(event.target.id==='edit-launch-form'){event.preventDefault();const form=new FormData(event.target);const args=String(form.get('arguments')||'').split(/\r?\n/).map(v=>v.trim()).filter(Boolean);const workingDirectory=String(form.get('workingDirectory')||'').trim()||null;perform(async()=>{acceptGames(await call('update_game_launch',{gameId:String(form.get('gameId')),executable:String(form.get('executable')).trim(),arguments:args,workingDirectory}));$('#dialog').close();render();notify('Launch settings saved.');});return;}
